...
//tambahan bagus diatas post wa
                                                    ...
                                                    // } 
                                                     ...
                                                     if (($product[0]['product_count'] === "Y") || ($product[0]['combo_product'] === "Y")) {
                                                            // Hitung jumlah pesanan berdasarkan array_sku atau jumlah
                                                            $jml = !empty($array_sku) ? count($array_sku) : $jumlah;
                                                            
                                                            
                                                            
                                                        
                                                            // Validasi jumlah pesanan agar tidak ada kelebihan hit
                                                            if (!is_array($array_sku) && $jml > 1) {
                                                                $jml = 1; // Pastikan hanya satu pesanan jika array_sku kosong
                                                            }
                                                            ...
                                                            for ($i = 0; $i < $jml; $i++) {
                                                                ...
                                                                curl_setopt($ch, CURLOPT_POST, 1);
                                                                curl_setopt($ch, CURLOPT_POSTFIELDS, $post_data);
                                                                curl_setopt($ch, CURLOPT_TIMEOUT, $curl_timeout); // Set timeout
        ...
        log_message('error', 'API call or data processing failed: ' . $e->getMessage());
        $this->session->set_flashdata('error', 'Terjadi kesalahan saat memproses pesanan. Silakan coba lag i.');
        return redirect()->to(str_replace('index.php/', '', site_url(uri_string())));
        }
                                                         ...
                                                         try{
                                                            if($product[0]['category_id'] == "4"){
                                                                ...
                                                                if(count($product_vouchers) > 0){
                                                                    if(isset($product_vouchers[0]['id'])){
                                                                        $id_product_voucher = $product_vouchers[0]['id'];
                                                                        $status = 'Success';
                                                                        $ket = $product_vouchers[0]['kode_voucher'];
                                                                    }else{
                                                                        $status = 'Processing';
                                                                        $ket = 'Voucher tidak tersedia. Silahkan kontak Admin!';
                                                                    }
                                                                }else{
                                                                    $status = 'Processing';
