    ...
    public function index($slug = null) {
            ...
            if (count($games) === 1) {
                ...
                if ($games[0]['slug'] === $slug) {
                    ...
                    if ($this->request->getPost('method') AND $this->request->getPost('product')) {
                        ...
                        log_message('info', 'Data Post User: ' . print_r($data_post, true));
                        ...
                        if ($games[0]['target'] != 'default') {
                            if (in_array($games[0]['slug'], ['promo-diamond-slow', 'promo-gift-skin', 'joki-ranked-mobile-legends',])) {
                                ...
                                if(isset($data_post['data']['user_id'])){
                                   $data_post['user_id'] = trim(htmlentities($data_post['data']['user_id']));   
                                }
                            }
                            // Passing checking target untuk product provider "Manual" dan category "Voucher"
                                if(!empty($data_post['product'])){
                                    $product = $this->M_Base->data_where('product', 'id', $data_post['product']);
                                    if($product[0]['category_id'] == "4" && $product[0]['provider'] == "Manual"){
                                        // Nothing To do Check Target
                                    }else{
                                    ...
                                    if (count($method) === 1) {
                                        if ($method[0]['status'] == 'On') {
                                            ...
                                            if ($method[0]['provider'] == 'Tripay') {
                                                ...
                                                log_message('info', 'EWallet Charge Parameters: ' . print_r($result, true));
                                                
                                               if ($result) {
                                                if ($result['success'] == 1) {
                                                    if (!empty($result['data'])) {
                                                        $payment_code = $result['data']['checkout_url']; // Default value
                                                        
                                                        if (!empty($result['data']['qr_url'])) {
                                                            $payment_code = $result['data']['qr_url'];
                                                        } elseif (!empty($result['data']['pay_code'])) {
                                                            $payment_code = $result['data']['pay_code'];
                                                        }
                                            
                                                        // Debugging tambahan
                                                        log_message('info', 'Full Data: ' . print_r($result['data'], true));
                                                        log_message('info', 'Selected Payment Code: ' . $payment_code);
                                                        
                                                        // Pastikan $payment_code tersedia untuk proses selanjutnya
                                                        if (empty($payment_code)) {
                                                            log_message('error', 'Payment code is empty after all checks');
                                                            $this->session->setFlashdata('error', 'Failed to get payment code');
                                                            return redirect()->to(str_replace('index.php/', '', site_url(uri_string())));
                                                        }
                                                    } else {
                                                        log_message('error', 'Data is empty');
                                                        $this->session->setFlashdata('error', 'Empty payment data');
                                                        return redirect()->to(str_replace('index.php/', '', site_url(uri_string())));
                                                    }
                                                } else {
                                            ...
                                            log_message('error', 'API call or data processing failed: ' . $e->getMessage());
        ...
        return redirect()->to(str_replace('index.php/', '', site_url(uri_string())));
                                                ...
                                                if ($method[0]['code'] == 'QR') {
                                                  ...
                                                  log_message('info', 'EWallet Charge Parameters: ' . print_r($ewalletChargeParams, true));  
                                                ...
                                                if ($this->users == false) {
                                                    ...
                                                    } elseif(!empty($data_post['user_id'])) {
                                                        $customer_no = $data_post['user_id'];
                                                    } else {
                                                        $customer_no = $data_post['wa'];
                                                    }
//tambahan bagus diatas post wa                     
                                                   /*if ($product[0]['provider'] === 'DF') {
                                                        try {
                                                            ...
                                                            if (($product[0]['product_count'] === "Y") || ($product[0]['combo_product'] === "Y")) {
                                                                ...
                                                                for ($i = 0; $i < $jml; $i++) {
                                                                    $id_sku_product = !empty($array_sku) ? $array_sku[$i] : $product[0]['sku'];
                                                                    ...
                                                                    $post_data = json_encode([
                                                                        ...
                                                                        'ref_id' => $ref_id,
                                                                        'sign' => md5($df_user . $df_key . $ref_id),
                                                                    ]);
                                                                    ...
                                                                    if (curl_errno($ch)) {
                                                                        ...
                                                                        log_message('error', "cURL error for Ref ID {$ref_id}: {$error_message}");
                                                                        $tmp_ket[$i] = 'Failed Order: Connection Error';
                                                                        curl_close($ch);
                                                                        continue;
                                                                    }
                                                                    ...
                                                                    //log_message('info', "Response from API for Ref ID {$ref_id}: " . print_r($result, true));
                                                                    ...
                                                                    if (isset($result['data'])) {
                                                                        ...
                                                                        log_message('error', "Invalid API response for Ref ID {$ref_id}");
                                                                        $tmp_ket[$i] = 'Failed Order: Invalid API Response';
                                                                    }
                                                                }
                                                ...
                                                try {
                                                    ...
                                                    // Modifikasi kondisi untuk menerima produk normal (non-combo)
                                                    $is_combo_or_count = ($product[0]['product_count'] === "Y") || ($product[0]['combo_product'] === "Y");
                                                    
                                                    if ($is_combo_or_count) {
                                                        // Kode untuk produk combo/count seperti sebelumnya
                                                        if (!empty($array_sku)) {
                                                            if (!is_array($array_sku)) {
                                                                $array_sku = explode(",", $array_sku);
                                                            }
                                                            $jml = count($array_sku);
                                                        } else {
                                                            $jml = $jumlah;
                                                        ...
                                                        // Kode untuk produk normal (non-combo)
                                                        $jml = 1; // Hanya 1 produk
                                                        $array_sku = [$product[0]['sku']]; // Gunakan SKU utama
                                                        $order_id_reference = [$order_id]; // Gunakan order_id utama sebagai reference
                                                    }
                                                    ...
                                                    for ($i = 0; $i < $jml; $i++) {
                                                        ...
                                                        log_message('info', "Info Post Data" . print_r($post_data, true));
                                                        ...
                                                        curl_close($ch);
                                                        $result = json_decode($result, true);
                                                         log_message('info', "Info Result games DF" . print_r($result, true));
                                                        if (isset($result['data'])) {
                                                            if ($result['data']['status'] === 'Gagal') {
                                                                $tmp_ket[$i] = $result['data']['message'];
                                                            } 
                                                            ...
                                                            // }
                                                                ...
                                                                if (isset($result['data']['status'])) {
                                                                    if ($result['data']['status'] == 'Pending') {
                                                                        $status = 'Pending';
                                                                        $tmp_ket[$i] = $result['data']['message'];
                                                                    } elseif (isset($result['data']['sn']) && $result['data']['sn'] != '') {
                                                                        $status = 'Success';
                                                                        $tmp_ket[$i] = $result['data']['sn'];
                                                                    }
                                                                }
                                                            ...
                                                            log_message('info', "Info Result games DF" . print_r($result, true));
                                                        log_message('info', "Info SN games DF" . print_r($tmp_ket[$i], true));
                                                ...
                                                }elseif($product[0]['provider'] == 'Unipin'){
                                                                                                          if (($product[0]['product_count'] === "Y") || ($product[0]['combo_product'] === "Y")) {
                                                                                                            try {
                                                                                                                // Tentukan ID produk dan kode game berdasarkan game
                                                                                                                $game = strtolower($games[0]['games']); // Pastikan game dalam format huruf kecil
                                                                                                                $game_mapping = [
                                                                                                                    'free fire' => ['api_code' => 'ff'],
                                                                                                                    'ml' => ['api_code' => 'ml'],
                                                                                                                    'pubg' => ['api_code' => 'pubg'],
                                                                                                                    'gi' => ['api_code' => 'gi'],
                                                                                                                    'zzz' => ['api_code' => 'zzz'],
                                                                                                                ...
                                                                                                                log_message('info', "Response game mapping " . json_encode($game_mapping));
                                                                                                        
                                                                                                                if (!isset($game_mapping[$game])) {
                                                                                                                    log_message('error', "Game tidak dikenali: {$game}");
                                                                                                                    return;
                                                                                                                }
                                                                                                        
                                                                                                                $id_produk = $product[0]['sku'];
                                                                                                                $region = $product[0]['region'];
                                                                                                                $api_game = $game_mapping[$game]['api_code'];
                                                                                                                ...
                                                                                                                for ($i = 0; $i < $jml; $i++) {
                                                                                                                    ...
                                                                                                                    if (!$ref_id || in_array($ref_id, $processed_ref_ids)) {
                                                                                                                        log_message('info', "Skipped Ref ID {$ref_id}: Missing or already processed.");
                                                                                                                        continue;
                                                                                                                    }
                                                                                                                ...
                                                                                                                $processed_ref_ids_store[] = $ref_id;
                                                                                                                    ...
                                                                                                                    $ch = curl_init();
                                                                                                                    curl_setopt($ch, CURLOPT_URL, 'https://api.dimunaz-pedia.id/unipinapi');
                                                                                                                    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                                                                                                                    curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
                                                                                                                    curl_setopt($ch, CURLOPT_HTTPHEADER, ['Content-Type: application/json']);
                                                                                                                    curl_setopt($ch, CURLOPT_POST, 1);
                                                                                                                    curl_setopt($ch, CURLOPT_POSTFIELDS, $post_data);
                                                                                                                    curl_setopt($ch, CURLOPT_TIMEOUT, 30); // Perpanjang timeout ke 10 detik
                                                                                                        
                                                                                                                    $result = curl_exec($ch);
                                                                                                        
                                                                                                                    if (curl_errno($ch)) {
                                                                                                                        log_message('error', "cURL error for Ref ID {$ref_id}: " . curl_error($ch));
                                                                                                                        $tmp_ket[$i] = 'Failed Order: Connection Error';
                                                                                                                        curl_close($ch);
                                                                                                                        continue;
                                                                                                                    }
                                                                                                                    ...
                                                                                                                    // **Hanya lanjutkan jika status sukses**
                                                                                                                    if (isset($result['status']) && $result['status'] === true && isset($result['info']) && $result['info'] === "Transaksi berhasil") {
                                                                                                                        // **Proses berikutnya jika transaksi sukses**
                                                                                                                        log_message('info', "Transaksi Ref ID {$ref_id} berhasil, melanjutkan eksekusi...");
                                                                                                        
                                                                                                                        // Simpan SN atau data lain
                                                                                                                        $status = 'Success';
                                                                                                                        $tmp_ket[$i] = $result['sn'];
                                                                                                                        
                                                                                                                    } else {
                                                                                                                        log_message('error', "Transaksi gagal untuk Ref ID {$ref_id}: " . ($result['info'] ?? 'Tidak diketahui'));
                                                                                                                        $tmp_ket[$i] = 'Failed Order: ' . ($result['info'] ?? 'Unknown error');
                                                                                                                    }
                                                                                                                }
                                                                                                                  $session->set('processed_ref_ids', $processed_ref_ids_store);
                                                                                                                  
                                                                                                                $ket = json_encode($tmp_ket);
                                                                                                                log_message('error', "Transaksi gagal untuk Ref ID {$ket}");
                                                ...
                                                }elseif ($product[0]['provider'] == 'Tokogar') {
                                                    try {
                                                        ...
                                                        for ($i = 0; $i < $jml; $i++) {
                                                            ...
                                                            try {
                                                                ...
                                                                log_message('info', "Tokogar API Response [{$statusCode}]: " . print_r($response, true));
                                                                ...
                                                                $tmp_ket[$i] = 'Failed: ' . $error_msg;
                                                                log_message('error', "Ref ID {$ref_id}: {$error_msg}");
                                                                continue;
                                                            }
                                                        }
                                                        ...
                                                        if (!empty($tmp_ket)) {
                                                            $ket = implode(' | ', $tmp_ket);
                                                            log_message('info', "Final transaction results: {$ket}");
                                                        ...
                                                        try {
                                                            $jml_user = $jumlah; // Contoh jumlah user, bisa diganti dengan nilai dinamis
                                                                   ...
                                                                   if ($result) {
                                                                        if (array_key_exists('statusCode', $result)) {
                                                                            ...
                                                                            $status = 'Success';
                                                                                ...
                                                                                $reference = ['merchant' => $this->M_Base->u_get('voca_merchant'), 'secret' => $this->M_Base->u_get('voca_secret')];
                                                                                $voca = [
                                                                                    'merchant' => $this->M_Base->u_get('voca_merchant'),
                                                                                    'secret' =>  $this->M_Base->u_get('voca_secret'),
                                                                                    'key' => $this->M_Base->u_get('voca_key'),
                                                                                ...
                                                                                $resultx = $this->M_Voca->trxdetail('GET',  $tmp_id_provider[$i], $reference, $voca);
                                                                                $tmp_ket[$i] = $resultx['data']['sn'];
                                                                        }
                                                                    } else {
                                                                        $tmp_id_provider[$i] = '';
        ...
        return redirect()->to(str_replace('index.php/', '', site_url(uri_string())));
                                                            ...
                                                            if($product[0]['combo_product'] == "Y"){
                                                                ...
                                                                for ($i=0; $i < $jml; $i++) {
                                                                    ...
                                                                    if ($response) {
                                                                        ...
                                                                        if ($response['error'] == false) {
                                                                            $status = 'Success';
                                                                            $tmp_ket[$i] = $response['data']['invoiceNumber'];
                                                                            
                                                                        } else {
                                                                            $tmp_ket[$i] = 'Failed : ' . $response['message'] . ' ';
                                                                        }

                                                                    } else {
                                                                       $tmp_ket[$i] = 'Gagal terkoneksi ke provider' . ' ';
                                                                    }
                                                                }
                                                                $ket = json_encode($tmp_ket);
                                                ...
                                                return redirect()->to(str_replace('index.php/', '', site_url(uri_string())));
                                                        ...
                                                        if(!empty($data_post['zone_id'])){
                                                            $post_data = json_encode([
                                                                ...
                                                                'input1' => $data_post['user_id'],
                                                                'input2' => $data_post['zone_id'],
                                                            ]);        
                                                        }else{
                                                            $post_data = json_encode([
                                                                'order_id' => $order_id,
                                                ...
                                                return redirect()->to(str_replace('index.php/', '', site_url(uri_string())));
                                                        ...
                                                        if(($product[0]['product_count'] == "Y") || ($product[0]['combo_product'] == "Y")){
                                                             ...
                                                             for ($i=0; $i < $jml; $i++) {
                                                                ...
                                                                $curl = curl_init();
                                                                curl_setopt_array($curl, array(
                                                                    CURLOPT_URL => 'https://v1.apigames.id/transaksi/http-get-v1?merchant='.$this->M_Base->u_get('ag-merchant').'&secret='.$this->M_Base->u_get('ag-secret').'&produk='.$id_sku_product.'&tujuan='.$customer_no.'&ref=' . $order_id_reference[$i],
                                                                    CURLOPT_RETURNTRANSFER => true,
                                                                    CURLOPT_ENCODING => '',
                                                                    CURLOPT_MAXREDIRS => 10,
                                                                    CURLOPT_TIMEOUT => 0,
                                            ...
                                            //By Pass if Product Provider "Manual" && Category "Voucher"
                                                if($product[0]['category_id'] == "4" && $product[0]['provider'] == "Manual"){
                                                    $target = $target_json = null;
                                                }
                                            ...
                                            // Update Total Sold dan Stock untuk Produk Flash Sale
                                            if($product[0]['fs'] == "Y"){
                                                if($status == "Success"){
                                            ...
                                            $email_smtp->setSubject("INOVICE DIMUNAZ PEDIA - $order_id");
                    ...
                    $selected_product_category_id = 2;
                    $array_uri = parse_url($_SERVER['REQUEST_URI']);
                    if(isset($array_uri['query'])){
                        $id_product = substr($array_uri['query'], strpos($array_uri['query'], "=") + 1);
                        $product_detail = $this->M_Base->data_where('product', 'id', $id_product);
                        if(isset($product_detail[0]['category_id'])){
                            $selected_product_category_id = $product_detail[0]['category_id'];
                        }    
                    }
                   
                    $iduser = $this->users['id'] ?? null;
                    $playerIds = [];
                    if ($iduser) {
                        // //$playerIds = $this->db->table('player_id')->where('user_id', $iduser)->get()->getResultArray();
                        // $playerIds = $this->db->table('player_id')
                        // ->where('user_id', $iduser) // Kondisi pertama
                        // ->where('games', $target[0]['target'])   // Kondisi kedua
                        // ->get()
                        // ->getResultArray();
                        
                        // Periksa apakah $target memiliki elemen dan properti 'target'
                            if (isset($target[0]['target'])) {
                                $playerIds = $this->db->table('player_id')
                                    ->where('user_id', $iduser) // Kondisi pertama
                                    ->where('games', $target[0]['target']) // Kondisi kedua
                                    ->get()
                                ...
                                // Handle jika $target tidak memiliki sistem target
                                $playerIds = $this->db->table('player_id')
                                    ->where('user_id', $iduser) // Hanya berdasarkan user_id
                                    ->get()
                                    ->getResultArray();
                            }
                    }
                    ...
                    $data = array_merge($this->base_data, [
                        ...
                        'product' => $product,
                        'selected_product_category_id' => $selected_product_category_id,
                        'target' => $target,
                        'popup' => [
                            ...
                            if ($voucher_error == null) {
                                ...
                                $total = (($price * $jumlah) - $diskon) + $fee;
                                 ...
                                 $target_alert = $this->M_Base->data_where('target', 'id', $games[0]['target']);
                                   ...
                                   $cektrx = $this->M_Base->data_where_array('orders', [
                                                    'user_id' => $data_post['user_id'],
                                                    'product_id' => $data_post['product'],
                                                    'target_json' => $data_post['target'],
                                                    'date'   => date('Y-m-d')
                                                ]);
                                   ...
                                   if (count($cektrx) > 0) {
                                         ...
                                         $csrf_token = csrf_hash();
                                            echo json_encode([
                                                ...
                                                'msg' => '
                                                    ...
                                                    <h2 class="fs-16">Transaksi Ganda Terdeteksi</h2>
                                                    <hr class="my-3">
                                                      <table class="w-100 mb-4">
                                                        ...
                                                        <tr id="tr-target">
                                                            ...
                                                            <th class="text-end">'.$data_post['target'].'</td>
                                                        </tr>
                                                    </table>
                                    ...
                                    if ($games[0]['cek_id_provider'] === 'omega' || $games[0]['cek_id_provider'] === 'kuropedia') {
                                    
                                        if ($data_post['user_id'] && $data_post['zone_id']) {
                                            if($games[0]['cek_id_provider'] == 'omega'){
                                                $content = @file_get_contents('https://develo.dimunaz-pedia.id/api/game/'.$games[0]['check_code'].'?id='.$data_post['user_id'].'&zone='.$data_post['zone_id'].'');
                                            }
                                            if($games[0]['cek_id_provider'] == 'kuropedia'){
                                                $content = @file_get_contents('https://checknick.dimunaz-pedia.id/api/game/'.$games[0]['check_code'].'?id='.$data_post['user_id'].'&zone='.$data_post['zone_id'].'');
                                            }
                                        } else {
                                            if($games[0]['cek_id_provider'] == 'omega'){
                                                $content = @file_get_contents('https://develo.dimunaz-pedia.id/api/game/'.$games[0]['check_code'].'?id='.$data_post['user_id'].'');
                                            }
                                            if($games[0]['cek_id_provider'] == 'kuropedia'){
                                                $content = @file_get_contents('https://checknick.dimunaz-pedia.id/api/game/'.$games[0]['check_code'].'?id='.$data_post['user_id'].'');
                                            }
                                        }
                                    ...
                                    if ($games[0]['cek_id_provider'] === 'kenzo' || $games[0]['cek_id_provider'] === 'apigame' || $games[0]['cek_id_provider'] === 'pln' || $games[0]['cek_id_provider'] === 'voca' || $games[0]['cek_id_provider'] === 'enivay') {
                                        ...
                                        /*  if ($games[0]['cek_id_provider'] === 'pln') {
                                         ...
                                         if ($games[0]['cek_id_provider'] === 'pln') {
                                            ...
                                            $df_key = $this->M_Base->u_get('digi-key');
                                            $customer_no = $data_post['user_id'];
                                            $sign = md5($df_user . $df_key . $customer_no);
                                            ...
                                            $ch = curl_init();
                                            curl_setopt($ch, CURLOPT_URL, 'https://api.digiflazz.com/v1/inquiry-pln');
                                            curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
                                            curl_setopt($ch, CURLOPT_HTTPHEADER, ['Content-Type: application/json']);
                                            curl_setopt($ch, CURLOPT_POST, 1);
                                            curl_setopt($ch, CURLOPT_POSTFIELDS, $inquiry_data);
                                            $result = curl_exec($ch);
                                            $result = json_decode($result, true);
                                            curl_close($ch);
                                            ...
                                            log_message('info','result inquery:' . print_r($result, true));
                                            log_message('info','username:' . print_r($username, true));
                                        }
                                       ...
                                       // Passing checking nick untuk product provider "Manual" dan category "Voucher"
                                        if($product[0]['category_id'] == "4" && $product[0]['provider'] == "Manual"){
                                            $csrf_token = csrf_hash();
                                            echo json_encode([
                                                'status' => true,
