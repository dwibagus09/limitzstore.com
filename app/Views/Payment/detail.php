		                    		...
		                    		<div class="card">
		                    			<div class="card-body eniv-card">
		                    				...
		                    				<!--<p style="font-size:12px"><= $orders['ket']; ?></p>-->
		                    				 <?php
                                            // Mengambil data dari $orders['ket']
                                            $ket = $orders['ket'];
                                            ...
                                            echo implode("<br><br>", $formattedItems);
                                            ?>
		                    	        </div>
		                    	    </div>
                    			</div>
						...
						<div class="col-md-7">
                			...
                			<div class="card">
                    			<div class="card-body eniv-card">
		                    				...
		                    				<div class="col-12" align="center">
		                    					...
		                    					<?php if (filter_var($orders['payment_code'], FILTER_VALIDATE_URL)): ?>
	                    					        <?php if (in_array('tripay', explode('.', $orders['payment_code'])) OR in_array('qr', explode('/', $orders['payment_code']))): ?>
														...
														<br />
														<a href="https://tripay.co.id/qr/<?= $explode[1]; ?>" class="btn btn-success" download ><i class="fa fa-download"></i> Unduh QR</a>
											   ...
											   <?php else: ?>
            										...
            										<br />
            										<a href="https://api.qrserver.com/v1/create-qr-code/?size=1080x1080&data=<?= $orders['payment_code']; ?>" class="btn btn-success mt-2 mb-2" download ><i class="fa fa-download"></i> Unduh QR</a>
                								    <?php endif ?>
                                    ...
                                    // Cek jika ini payment link (bukan QR Code)
                                    if ((strpos($payment_code, 'tokopay') !== false || 
                                         strpos($payment_code, 'tripay') !== false ||
                                         strpos($payment_code, 'xendit') !== false ||
                                         strpos($payment_code, 'dana') !== false ||
                                         strpos($payment_code, 'duitku') !== false ||
                                         strpos($payment_code, 'shp') !== false ||
                                         strpos($payment_code, 'airpay') !== false) &&
                                         strpos($payment_code, 'qrcode') === false &&  // Pastikan bukan QR Code
                                         strpos($payment_code, 'qr.') === false &&    // Pattern alternatif untuk QR
                                         strpos($payment_code, 'qr/') === false) :    // Pattern alternatif lainnya
                                    ?>
                                        <p class="mb-2" style="font-size: 12px;text-align: center;">Klik tombol dibawah untuk melakukan pembayaran</p>
                                        <a href="<?= htmlspecialchars($payment_code, ENT_QUOTES, 'UTF-8') ?>" class="btn btn-success mb-4" style="width:100%">Bayar Sekarang</a>
                                    <?php endif ?>
                    			...
                    			<div class="col-md-12">
		                    		<div class="card">
		                    			<div class="card-body eniv-card">
                                            ...
                                            foreach ($ketArray as $item) {
                                                if (!empty($item)) {
                                                    // Bersihkan karakter escape dan spasi ekstra
                                                    $item = str_replace('\/', '/', $item); // Ubah \/ menjadi /
                                                    $item = trim($item); // Hapus spasi di awal/akhir
                                            
                                                    // Hapus "Transaksi Pending" dan format elemen lainnya
                                                    $formattedItem = str_replace(
                                                        ['Transaksi Pending', '/nick: ', '/ref: '], // String yang akan dihapus/diganti
                                                        ['Sukses', 'Nick: ', 'Ref: '],                    // Pengganti (kosong untuk menghapus)
                                                        $item
                                                    );
