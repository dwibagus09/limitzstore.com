                			...
                			<div class="card">
                    			<div class="card-body eniv-card">
	                    			...
	                    			<div class="eniv-card-payment">
	                    				<table class="w-100">
	                    					<tr>
	                    						<td class="pt-2 pb-3">Metode</td>
	                    						<th class="text-end">
	                    							<img src="<?= base_url(); ?>/assets/images/method/<?= $orders['method_image']; ?>" class="eniv-method-image" alt="">
	                    						</th>
	                    					</tr>
	                    					<tr>
	                    						<td class="pt-2 pb-3">Pembayaran</td>
	                    						<th class="text-end" style="white-space: pre-wrap;">
                    								...
                    								<?php else: ?>
                    								    ...
                    								    <?php else: ?>
                										<strong><p class="mb-0">Scan QR</p></strong>
                										<img src="https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=<?= $orders['payment_code']; ?>" width="180" class="mt-3">
                										<br />
                										<a href="https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=<?= $orders['payment_code']; ?>" class="btn btn-success mt-2 mb-2" download ><i class="fa fa-download"></i> Unduh QR</a>
                    								    <?php endif ?>
	                    			...
	                    			<div class="mt-4">
	                    			    <?php if (in_array('tokopay', explode('.', $orders['payment_code'])) OR in_array('tripay', explode('.', $orders['payment_code'])) OR in_array('xendit', explode('.', $orders['payment_code'])) OR in_array('dana', explode('.', $orders['payment_code'])) OR in_array('duitku', explode('.', $orders['payment_code'])) OR in_array('airpay', explode('.', $orders['payment_code']))): ?>
		...
		<?php if ($orders['status'] == 'Pending'): ?>
		<script>
			var second_expired = new Date("<?= date('M d, Y G:i:s', strtotime($orders['date_create']) + 60*60*24); ?>").getTime();
